...
module.exports = {
  ...
  createLogTimeSheetManagement: async (data, callback) => {
    try {
      ...
      console.log("today's date",date.toISOString(), dateTime.toString())

      let dbTime = await request.query(`
        SELECT 
    CASE 
        WHEN cast('${dateTime.toString().split('T')[0]}' as Date) = CAST(GETDATE() AS DATE) OR cast('${dateTime.toString().split('T')[0]}' as Date) = DATEADD(DAY, 1, CAST(GETDATE() AS DATE)) 
        THEN 'Accepted' 
        ELSE 'Not Accepted' 
    END AS status;
    `)

      console.log("db todays time",dbTime.recordset[0].status)

      if(dbTime.recordset[0].status !="Accepted"){
